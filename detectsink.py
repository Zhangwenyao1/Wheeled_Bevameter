...
def detect():
    ...
    try :
        while (index<4):
            ...
            color_image = cv2.imread('/home/zwy/Desktop/DynamixelSDK-3.7.31/python/tests/protocol2_0/bright1_ColorC.png')
            ...
            # gray = cv2.cvtColor(color_image, cv2.COLOR_BGR2GRAY)
            # cv2.imshow('col ', color_image)  # 开运算得出的图片
            # cv2.waitKey(0)
        ...
        # 进行apriltag检测，得到检测到的apriltag的列表
            ...
            for tag in tags:
                ...
                pts1 = np.float32([tag.corners[0], tag.corners[1], tag.corners[2], tag.corners[3]])
                pts2 = np.float32([[500, 500], [500, 600], [600, 600], [600, 500]])
                # pts2 = np.float32([[400, 400], [400, 600], [600, 600], [600, 400]])
                M = cv2.getPerspectiveTransform(pts1, pts2)
                dst = cv2.warpPerspective(color_image, M, (1500,1500)) # 映射
                cv2.imshow('dst',dst)    
                cv2.imwrite('dst.png',dst)
                color_image = dst
                # cv2.waitKey(0)
                # tag.corners[0] = [400, 400]
                # tag.corners[1] = [400, 600]
                # tag.corners[2] = [600, 600]
                # tag.corners[3] = [600, 400]
                tag.corners[0] = [500, 500]
                tag.corners[1] = [500, 600]
                tag.corners[2] = [600, 600]
                tag.corners[3] = [600, 500]
                image = cv2.cvtColor(color_image, cv2.COLOR_BGR2HSV)
                img_H = image[:, :, 0]
                img_S = image[:, :, 1].copy( )
                img_I = image[:, :, 2]
                img = dst
                cv2.imshow('S', img_S)  # 开运算得出的图片
                ...
                '''形态学'''
                (thresh2, im_bw2) = cv2.threshold(img_S, 0, 200, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
                cv2.imshow('otsu2', im_bw2)
                cv2.imwrite('ostu.png',im_bw2)
                # cv2.waitKey(0)
                ...
                # cv2.waitKey(1)
                '''边缘提取算法，此处使用Canny算子'''
                edges = cv2.Canny(img_open, 100, 200)  # 参数:图片，minval，maxval,kernel = 3
                cv2.imshow('detect_edge',edges)
                cv2.imwrite('detect_edge.png',edges)
                # cv2.waitKey(1)
                if (tag.corners[1]).shape == (2,) :
                    ...
                    # print('point0',point0)
                    # print('point1',point1)
                    # diameter = distanceofpint(pc1, pc2) / 0.0268 * 0.0946# 小车轮
                    diameter = distanceofpint(pc1, pc2) / 0.0268 * 0.1305
                ...
                for i in range(int(0), int(h)) :
                    for j in range(0, int(w)) :
                        if (((i - center[0])**2 + (j - center[1])**2) <= radius**2) and (edges[j, i] > 10) :
                            edges[j-1:j+1, i-1:i+1] = 240
                            color_image[j-2:j+2, i-2:i+2]=[0,0,0]
                            x.append(i)
                            y.append(j)
                edges[edges > 250] = 0
                x = np.array(x)
                ...
                # 使用文件回归对参数进行拟合
                    ...
                    radius = DST1/(length/2 * (2**0.5)) * 0.1376 /2 #大车轮
                    # radius = DST1/(length/2 * (2**0.5)) * 0.09556/2 #小车轮
                    ...
                    # cv2.line(color_image, tuple(cross_point1.astype(int)), tuple(cross_point2.astype(int)), (0, 0, 0),
                    #         3)
                    # cv2.line(color_image, tuple(p1.astype(int)), tuple(p2.astype(int)), (255, 255, 255),
                        # 2)
            ...
            cv2.imwrite('嘈杂环境edges.png', edges)
            cv2.imwrite('嘈杂环境彩色图像.png',color_image)
            key = cv2.waitKey(0)
            print('沉陷量', sink)
